...
namespace JesseStiller.TerrainFormerExtension {
    public static class Utilities {
        ...
        internal static Vector3 GetClosestPointOnLine(Vector3 point, Vector3 lineStart, Vector3 lineEnd) {
            ...
            float aToPDotAToB = Vector3.Dot(lineStartToPoint, lineStartToEnd);
            float t = aToPDotAToB / lineStartToEnd.sqrMagnitude;

            return Vector3.LerpUnclamped(lineStart, lineEnd, t);
        }
        ...
        internal static string GetLocalPathFromAbsolutePath(string absolutePath) {
            int indexOfAssets = absolutePath.IndexOf("Assets", StringComparison.Ordinal);

            if(indexOfAssets == -1) {
                throw new ArgumentException("The 'assetsPath' parameter must contain with 'Assets/'");
            }
