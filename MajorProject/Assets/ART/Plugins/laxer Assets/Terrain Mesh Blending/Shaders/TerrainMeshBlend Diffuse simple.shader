...
Shader "Custom/TerrainMeshBlend/Diffuse simple" {
	Properties {
		...
		_BlendExponent ("Blend Gradient", Range(0,1)) = 0
	...
	ENDCG
		...
		void surf (Input IN, inout SurfaceOutput o) {
			...
			o.Normal = normalize(float3(nmap.x*2-1,nmap.y*2-1,nmap.z*2-1));
			o.Alpha = lerp(c.a,c.a*c.a*c.a,_BlendExponent);
		}
		ENDCG
	} 
	FallBack "Diffuse"
