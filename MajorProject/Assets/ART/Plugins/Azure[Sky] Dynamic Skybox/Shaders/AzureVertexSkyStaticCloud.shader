Shader "Azure[Sky]/Vertex/Sky Static Clouds"
{
	SubShader
	{
		...
		Pass
		{
			...
			uniform sampler2D   _Azure_MoonTexture, _Azure_StaticCloudTexture;
			...
			float4 frag (v2f IN) : SV_Target
			{
				...
				float4 cloudTexture = tex2D(_Azure_StaticCloudTexture, uv);
				       //cloudTexture = pow(cloudTexture,_Azure_GammaCorrection);
				...
				float3 cloud      = lerp(_Azure_StaticCloudColor2, _Azure_StaticCloudColor1 * _Azure_StaticCloudMultiplier, cloudTexture.r * cloudTexture.r);
				       cloud      = pow(cloud,2.2);
				       cloud     += IN.MoonBright.rgb * 1.25;
				       cloud     += IN.MieColor.rgb;
				...
				float3 OutColor  = IN.OutColor.rgb + max(0.0,((((Stars + MilkyWay)* (1.0 - cloudAlpha) * _Azure_StarfieldColorBalance) * IN.OutColor.a) * _Azure_StarfieldIntensity) * moonMask + MoonColor* (1.0 - cloudAlpha));
				       OutColor  = lerp(OutColor, cloud, cloudAlpha * IN.OutColor.a);
