	...
	SubShader
	{
		...
		Pass
		{
			...
			v2f vert (appdata v)
			{
    			...
    			o.MoonPos      = mul((float3x3)_Azure_MoonMatrix,v.vertex.xyz) * (25 * saturate(1 - _Azure_MoonDiskSize));
			...
			float4 frag (v2f IN) : SV_Target
			{
				...
				//Output.
				//-------------------------------------------------------------------------------------------------------
				float3 OutColor  = skyColor.rgb + max(0.0,((((Stars + MilkyWay) * _Azure_StarfieldColorBalance) * skyColor.a) * _Azure_StarfieldIntensity) * moonMask + MoonColor);
