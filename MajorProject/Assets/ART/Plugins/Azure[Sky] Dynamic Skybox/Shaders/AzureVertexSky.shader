		...
		Pass
		{
		  ...
		  //#pragma target 3.0
			...
			uniform float  _Azure_Pi316, _Azure_Pi14, _Azure_Kr, _Azure_Km, _Azure_SunIntensity, _Azure_Pi, _Azure_Exposure, _Azure_SkyDarkness,
			               ...
			               _Azure_SunDiskPropagation, _Azure_LightSpeed;
			...
			v2f vert (appdata v)
			{
				...
				float4 skyColor      = float4(inScatter + nightSky, saturate(fex.b * (viewDir.y + 0.1)));
				       skyColor.rgb += max(0.0, moonBright * moonRise);

				//Outputs.
				//-------------------------------------------------------------------------------------------------------
				o.OutColor = skyColor;//skyColor.a = Horizon extinction of stars, moom and deep space.
			...
			float4 frag (v2f IN) : SV_Target
			{
				...
				//Output.
				//-------------------------------------------------------------------------------------------------------
				float3 OutColor  = IN.OutColor.rgb + max(0.0,((((Stars + MilkyWay) * _Azure_StarfieldColorBalance) * IN.OutColor.a) * _Azure_StarfieldIntensity) * moonMask + MoonColor);
				       OutColor += sunDisk * sunMask;
